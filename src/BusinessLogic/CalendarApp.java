	...
	public void addYear(int yearNum) {
		...
		for(int i = 1; i<=12; i++) {
			...
			for(int j = 1; j<=31; j++) {
				...
				for(int k = 1; k <=24; k++) {
					hours[k-1] = new Hour(k, 0.0, j, i, yearNum);
					days[j-1].addHour(hours[k-1]);
					
					//adding an hour to the database
					try {
						Database.addHourDB(k, 0.0, j, i, yearNum);
					} catch (SQLException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
				...
				try {
					Database.addDayDB(days[j-1].getDayOfWeek(), yearNum, i, j);
				} catch (SQLException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			...
			try {
				Database.addMonthDB(months[i-1].getMonthName(), i, months[i-1].getNumOfDays(), yearNum);
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
